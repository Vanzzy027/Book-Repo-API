...
let books: Book[] = [
  { id: 1, title: 'The Hono Guide', author: 'Evan', description: 'Learn Hono', published: '2001-04-21', pages: 120 },
...
const deleteBook = (c: any) => {
  ...
  const booksIndex = books.findIndex(u => u.id === id)
  
    if(booksIndex === -1){
    return c.json({error:"Book not found"})
  }

  const deletedUser = books.splice(booksIndex,1)
  return c.json({message: 'Book deleted successfully', book: deletedUser[0]})
}
